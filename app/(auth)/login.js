...
export default function LoginScreen() {
    ...
    const handleDemoLogin = async (role) => {
        ...
        if (creds) {
            ...
            // Auto-submit after a tiny delay to let state update (or just call login directly)
            setLoading(true);
            try {
                await login(creds.email, creds.pass);
            } catch (e) {
                Alert.alert("Demo Failed", "Have you run the Seed Script yet? " + e.message);
                setLoading(false);
            }
        }
    };
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            // No need to navigate manually - the RootLayout useEffect will handle it!
        } catch (e) {
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
            ...
            {isDemoMode && (
                <View style={styles.demoBox}>
                    ...
                    <TouchableOpacity style={[styles.demoBtn, { backgroundColor: '#E67E22' }]} onPress={() => handleDemoLogin('engineer')}>
                        <Text style={styles.demoText}>👷 Field Engineer</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={[styles.demoBtn, { backgroundColor: '#8E44AD' }]} onPress={() => handleDemoLogin('qa')}>
...
const styles = StyleSheet.create({
    ...
    demoBtn: { padding: 12, borderRadius: 6, marginBottom: 8, alignItems: 'center' },
