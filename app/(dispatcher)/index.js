...
export default function DispatcherInbox() {
 ...
 // 1. Open Citizen Evidence
  ...
  const sla = getSLAStatus(selectedTicket ? selectedTicket.createdAt : Date.now()); 
  ...
  const potentialDuplicates = tickets.filter(t => 
    t.id !== selectedTicket.id && // Not self
    t.status !== 'resolved' &&    // Only open tickets
    getDistanceKm(
    ...
    // Check if assignment is allowed for this specific ticket
    ...
    return (
      ...
      {potentialDuplicates.length > 0 && (
        <View style={styles.duplicateBanner}>
          ...
          <TouchableOpacity style={styles.mergeBtn} onPress={() => alert("Merge Logic: Mark selected as parent, close others.")}>
        ...
        <View style={styles.actionBar}>
          ...
          >
            <Text style={styles.btnText}>
              {isAssignable ? '👷 Assign Engineer' : '🔒 Locked'} 
            </Text>
          </TouchableOpacity>
        </View>
